...
int UBlox::getConnectionStatus()
{
    ...
    {
    case UBLOX_STATE_CONNECTED:
        return UBLOX_CONN_CONNECTED;
    case UBLOX_STATE_CONNECTING:
    case UBLOX_STATE_REGISTERED:
        return UBLOX_CONN_REGISTERED;
    case UBLOX_STATE_NO_SIM:
        return UBLOX_CONN_ERR_SIM;
    case UBLOX_STATE_OFF:
        return UBLOX_CONN_ERR_OFF;
    case UBLOX_STATE_UNREGISTERED:
        return UBLOX_CONN_ERR_SIGNAL;
    default:
        return UBLOX_CONN_ERR_UNREGISTERED;
    }
}
...
bool UBlox::parse_sms_pdu(const char *fullpdu, sms_event &parsed_sms)
{
    ...
    if (strncmp(p, "0000", 4) != 0)
        return false; //protocol/encoding
    p += 4;
...
bool UBlox::write(int socket, const uint8_t *content, int length)
{
    ...
    if (socketnum == -1)
        return false;
    if (sockets[socketnum].type != SOCKET_TYPE_ACTIVE)
        return false;
...
bool UBlox::modemHasData()
{
    return modem->checkAvailable();
}
