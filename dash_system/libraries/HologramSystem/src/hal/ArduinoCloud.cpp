...
bool ArduinoCloud::convertToLocalTime(rtc_datetime_t &dest, const timestamp_tz &source)
{
    ...
    if (!ublox->getNetworkTime(networktime))
        return false;
    uint32_t seconds = getSecondsUTC(source);
...
bool ArduinoCloud::downloadOTA(const char *url, const char *destination)
{
    ...
    ublox->uhttp(1, 1, host);
    ublox->uhttp(1, 2, user);
    ublox->uhttp(1, 3, pass);
    ublox->uhttp(1, 4, "1");
    ublox->uhttp(1, 5, "80");
    return ublox->httpRequest("1", "1", path, destination);
}
